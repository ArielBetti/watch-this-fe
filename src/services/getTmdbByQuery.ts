...
export const getTmdbByQuery = async (query?: string): Promise<TTmdbMovies | undefined> => {
  ...
  }).get<TTmdbMovies>(`${ENDPOINTS.searchMovieByQuery}${params}${query}`);

  Promise.resolve(data);

  const tvSearch = await requester({
    baseURL: ENDPOINTS.baseUrl,
  }).get<TTmdbMovies>(`${ENDPOINTS.searchTvByQuery}${params}${query}`);
